...
func kvExactRuntimeValidate(engine string, kvExactMode uint8, modelName, apiMode, profileID string,
	...
	if eng == "vllm" {
		// Decided by the gateway's own launch environment, not by anything
		// in the request: the body is well-formed and no field a client can
		// change will make it succeed. Typed so the API layer answers 412
		// rather than a 400 that tells the client its input was malformed --
		// on a gateway launched without the seed EVERY KV-exact rule is
		// refused, and a bare 400 makes an unprovisioned gateway
		// indistinguishable from an operator's typo.
		//
		// The predicate lives in common so the capability surface reports
		// readiness from THIS check rather than a second copy of it: a
		// surface that says ready while admission refuses would be worse
		// than no surface. The messages are the operator-facing answer and
		// are deliberately unchanged.
		if perr := cmn.KvExactSeedPrecondition(deps.getenv); perr != nil {
			return res, perr
		}
	}
