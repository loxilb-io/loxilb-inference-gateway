...
echo SCENARIO-vllm-httpproxy-CHWBL-Level1
...
if ([ "$D1" -ge 8 ] && [ "$D2" -eq 0 ]) || ([ "$D2" -ge 8 ] && [ "$D1" -eq 0 ]); then
    ...
    echo "  ✗ CHWBL L1 inconsistent: requests split (D1=$D1, D2=$D2)"
    code=1
fi
