...
> **Scope:** SGLang integration on branch `` — the
...
> (`kvEngineType`/`kvDpRankCount`), the `KvEventSource` multi-DP-rank subscriber fan-out with
>  staleness fix, cross-VIP isolation (`kv_svc_id` threading), zero-hit
> watchdog, and the two-VIP coexistence CICD scenario.
> **Status:** code complete through plans (all committed). **Not yet
...
(the tier ladder and the P/D-only constraint this phase relaxes),
[11 — Configuration & tuning](11-hierarchical-kv-routing-config-tuning.md),
the internal A/B runbook (the wave-4 live A/B execution checklist).
...
The load-bearing constraint this phase removes: doc 10 §2's rule that *Tier 1.5 is
reachable only inside the P/D ladder*. SGLang serves single-role, so opened a
**second, additive entry** into KV-exact selection: `kvExactMode=3`
(`KV_EXACT_MODE_SINGLE_ROLE`, `loxilb-ebpf/common/sockproxy.h:435`). A plain `mode:4`
...
```
 ...
 SGLang clients ─HTTP─▶ VIP-B :9010   rule: mode 4 fullproxy, kvExactMode=3, engine=sglang     │
                       │   single-role sibling branch (sockproxy_ep.c:741)                     │──▶ SGLang EPs .7/.8/.9:30000
...
| Layer | Code | What added |
|---|---|---|
| **Request decision** (C) | `loxilb-ebpf/common/sockproxy_ep.c:741` (single-role branch), `sockproxy_kv_exact.c` | Sibling else-if beside the P/D block; SGLang hash arm; candidate-mask widening; `kv_svc_id` at the CGO call |
...
| **Contract fixtures** | `cicd/vllm-kvcache-routing-cpu/sglang_hash_core.py`, `scripts/compute_sglang_hash_refs.py` | ONE Python hash source of record + committed parity vectors pinned to sglang `d8ef76682e` |
...
- **The single-role path inherits the whole post-91 selection stack.** Because it calls the
...
- **Hash parity is still the whole game**, but the contract is a different one (§4), and
   adds a runtime tripwire for silent parity failure (§7) that doc 08's vLLM path
  never had.
...
99-RESEARCH established that "decouple Tier 1.5 from P/D" is not one gate but **three** —
and that the third was invisible until hot-spot post-mortem. All three opened in
plans.
...
(`sockproxy_ep.c:632`, doc 10 §2). attached a **sibling** `else if` at that
block's closing brace (`sockproxy_ep.c:741`):
...
Inside `pd_kv_exact_select` exactly one guarded change was needed — the candidate-mask
build (`sockproxy_kv_exact.c:837-841`) admits **all** EPs at mode 3 (single-role services
have no roles; `ep_role[]` is all-zero):
...
inventories. The fix is again a sibling branch (`rules.go:3586`), fanning out through a
pure, unit-testable contract function:

```go
// rules.go:2552 — the fan-out contract
...
(`rules.go:2537`) mirrors the C define. Teardown is already generic and service-scoped —
rule delete calls `KvSubscriberStopAll(uint32(rule.ruleNum))` (`rules.go:3782`) for both
modes.

Two **config-time validations** land beside the gate (`rules.go:2867-2885`, placed before
the eRule lookup so they cover create *and* update):
...
never binds → pure overlap argmax → the shared-prefix hot-spot, resurrected exactly as in
 (where a dead metrics feed made the blend run blind).

The single-role branch therefore adds a full load lifecycle:

- **Increment on hit** (`sockproxy_ep.c:775`): `atomic_fetch_add(&pd_ep_loads[kv_sr_ep].
  active_conns, 1)` plus `pfe->kv_sr_load_held = 1`, `pfe->kv_sr_ep_idx = kv_sr_ep`.
- **Single-owner atomic-claim decrement**: exactly one of two owners releases the hold by
  ...
  - the **backend connect-failure path** (`sockproxy_ep.c:850`), or
  - **`pd_cleanup()`** at generic teardown (`sockproxy_http.c:3516-3531`) — the *same*
    ...
    concurrent teardown races (the class) cannot double-decrement.
...
> **Related A1 finding :** `kv_warmup_start` has **no production writer** anywhere
> (only `test_kv_exact.c` stamps it) — GUARD_B warmup is **inert in production for both
> the P/D and single-role paths** today. Documented at the field declaration
> (`sockproxy.h:542-549`); deliberately *not* stamped, because stamping would change
...
compiled pybind module, so it is never imported; the regen script and the mock publisher
both import *this* module so vectors and publisher can never drift).

| Contract element | Definition | loxilb implementation |
|---|---|---|
| Block input | raw bytes `[32-byte parent digest, ONLY if present] ‖ token0_LE4 ‖ token1_LE4 ‖ …` per `--page-size` tokens — **no CBOR envelope** (`hash_binding.cpp` hashes the page's uint32 words directly; x86_64 memory order == 4-byte LE per token) | `kv_hash_sglang_block()` (`sockproxy_kv_exact.c:475`): `SHA256_Update(parent, 32)` iff `has_parent`, then per-token LE4 bytes |
| Hash algorithm | SHA-256 (32-byte digest), always | `KV_HASH_SHA256_SGLANG 2` (`sockproxy_kv_exact.h:40`) — value 2 is free in the *hash* enum; the NATS "2" reservation is on the *mode* enum only |
| First-block parent | **NONE** — block 0 hashes bare tokens. There is no `NONE_HASH`, no seed, no zero-parent bytes. Contrast vLLM: `NONE_HASH` derived from `PYTHONHASHSEED` (doc 08 §4) — the whole `LLB_KV_NONE_HASH_SEED` machinery does not exist on this path | `has_parent=false` skips the parent update entirely (dedicated loop, `sockproxy_kv_exact.c:535`) |
...
| Published value | `hash_str_to_int64`: `v = int(hexdigest[:16], 16)` — the **FIRST 8 digest bytes as a big-endian uint64** — then signed-int64 wrap: `v - 2^64 if v >= 2^63 else v` | `memcpy(out, digest_full, 8)` (**first** 8, `sockproxy_kv_exact.c:551` region) — the exact **inverse** of the vLLM `digest[-8:]` slice. The C parity test self-checks per block that `BE(digest[-8:])` *differs* from the published value, so a last-8 mis-implementation cannot silently pass any vector |
...
The dedicated SGLang loop inside `kv_compute_block_hashes` (`sockproxy_kv_exact.c:535`)
...
Committed constants, regenerated **once** from the pinned checkout by
`scripts/compute_sglang_hash_refs.py` (which imports `sglang_hash_core` and does zero
digest arithmetic itself; output re-run bit-exact; pin commit stamped in every consumer's
header — do not hand-edit, do not update the checkout mid-phase). Five vectors:
...
consecutive port with its own independent seq counter. (plan) wraps —
never rewrites — the shipped subscriber of doc 08 §3.2/§3.5:

### 5.1 The seam and the fan-out

- **`KvEventSource`** (`ai_kv_subscriber.go:512`) is the transport-agnostic interface the
  loop consumes; it *embeds* the existing `kvZmqSubscriber` seam (so every pre-99 reconnect
  test still mock-injects unchanged). `newKvEventSource(ctx, engine, addr)` (`:523`) is
...
- **`KvSubscriberStartRank(serviceID, epIdx, rank, epIP, port, algo)`** (`:898`) is the
  ...
  keyed by **`kvEpRankKey{epIdx, rank}`** (`:306`) so N rank goroutines get N cancels
  (`KvSubscriberStopAll` iterates the composite keys; teardown-clean test pins
  zero leaked goroutines). `KvSubscriberStart` survives as a thin rank-0 wrapper (`:887`)
...
- **Rank ports:** `rules.go` wraps *both* gate paths (mode-1 and mode-3) in
  ...
  (`rules.go:3582`, `:3604-3606`), with `dpRanks = kvDpRankCount; 0 ⇒ 1`. Default 1
...
The pre-99 model kept `lastSeq` on the shared inventory — N ranks with independent seq
counters interleaving into one detector would false-"gap" on every cross-rank hop and
spuriously clear. Now:

- `rankLastSeq` is **goroutine-local** per rank (`runKvSubscriberLoopRank`,
  `ai_kv_subscriber.go:609`). Rank 0 seeds from `inv.lastSeq` (back-compat with pre-seeded
  test baselines) and mirrors progress back; ranks > 0 start at −1 and never touch the
  shared field.
- The reconnect KEEP/CLEAR resync (`kvResyncDecision`, `:118`; forward tolerance
  `kvSeqResumeWindow = 64`, `:96`) keys on the rank's **own** state — a rank-1 blip cannot
  clear warmth rank 0 built (the `TestKvMultiRankIsolatedResync` regression).

### 5.3 The staleness fix (— the folded todo's true residue)
...
left **phantom hashes** in the inventory (the "KV subscriber does not recover from EP
restart" todo, 2026-04-20). closes it: a gap with `replay == nil` now runs the same
conservative decision and emits a **structured marker** (`ai_kv_subscriber.go:729-734`):
...
the **whole** shared EP inventory (`:765`):
...
inventories were rejected as complexity without a proven need — revisit only if wave-4
shows measurable warmth loss.
...
```
Go r.ruleNum ──(DpLBRuleMod, dat.ca.cidx — dpebpf_linux.go:1575)──▶ pval->_id
    ──(llb_conv_nat2proxy)──▶ proxy_add_entry arg->_id
    ──(stamped at BOTH copy sites, sockproxy_http.c:1963 / :2364)──▶ tepval->kv_svc_id
    ──(pd_kv_exact_select CGO call, sockproxy_kv_exact.c:898)──▶ llb_ai_kv_best_worker(..., svcID)
    ──(Go, kvSvcScanTargets — ai_kv_subscriber.go:360)──▶ kvServices[svcID] single lookup
```

`kvServices` is keyed by `uint32(r.ruleNum)` (`rules.go:3568/:3596`) — exactly the value
that arrives. The CGO signature change was twin-lockstep (Go `//export` at
`ai_kv_subscriber.go:1259`, C prototype `sockproxy_ai_gw.h:392`, call site, and both stubs
in ONE commit — `kv_weight` precedent).
...
engineered to FAIL without this filter — and plan runs a one-shot revert-mutation
...
"KV-exact". vLLM has the same failure class (doc 08 §6.5's silent-failure decoder), but
 makes it a first-class runtime signal instead of a runbook checklist. The
watchdog is engine-agnostic — it also catches vLLM parity misconfig.

**Mechanism** (`kvZeroHitWatchdog`, `ai_kv_subscriber.go:1096`, evaluated once per scanned
service on the selector exit path at `:1226`):
...
The metric: `loxilb_pd_kv_zero_hit_watchdog_total{service_id}` (lazy CounterVec,
`api/prometheus/sockproxy_metrics.go:429`; `service_id` = the rule number, keeping two-VIP
scenarios attributable per arm; increment via `IncKvZeroHitWatchdog` `:823`, test getter
`KvZeroHitWatchdogValue` `:831`). No backend HTTP call anywhere — the watchdog is
data-path-only by decision.

**Operational contract (wave 4):** the A/B runbook makes this a HARD per-window gate for
the loxilb arm — `tier15_hits_total` delta ≠ 0 **AND** `zero_hit_watchdog_total` delta
== 0, else the arm is VOID ("every experiment self-confirms it FIRED").

---

## 8. Configuration

### 8.1 REST surface (additive fields, `api/swagger.yml:7077-7126`)

```jsonc
{
  "serviceArguments": {
    "mode": 4,                     // fullproxy — REQUIRED for kvExactMode 3 (validated)
    "kvExactMode": 3,              // 0=off, 1=zmq P/D, 2=nats(reserved), 3=zmq single-role
...
**The `kvHashAlgo` omission is deliberate and currently the only correct spelling via
REST:** the swagger enum still lists only `["sha256_cbor", "xxhash_cbor"]` — there is no
`sha256_sglang` enum value. The rule at the single dpebpf mapping site
(`dpebpf_linux.go:1773-1789`) does the work: `KvHashAlgo == "" && KvEngineType == "sglang"`
⇒ `kv_hash_algo = 2` (`KV_HASH_SHA256_SGLANG`); an explicit algo always wins (so
`xxhash_cbor` + sglang would be honored as 1 — and never match anything). Engine mapping:
`"sglang"` ⇒ `kv_engine_type = 1`; rank `0 ⇒ 1`.
...
Fleet specifics (ports, co-residency memory split, `/health` + `/health_generate` gating,
page-size read-back, the `:5557`-collision reason the co-resident window uses `:5561`) are
scripted in your provisioning tooling — do not improvise them.

### 8.3 The engine guards

| Guard | Behavior | Where |
|---|---|---|
| Engine allowlist | `kvEngineType` ∈ {`""`, `"vllm"`, `"sglang"`} — unknown strings are **rejected**, never silently vllm | `kvEngineConfigValidate`, `rules.go:2596` |
| Rank bounds | `kvDpRankCount` ≤ 8 (caps the port-range walk) | same |
| **Immutability** | engine change on a live rule ⇒ `RuleExistsErr` + exact message `lbrule-exist error: cant modify rule kv engine type (delete and recreate)`; `""`≡`"vllm"` equivalence honored. Deliberately **absent** from the change-detect OR-chain — a change must REJECT, never trigger a ruleChg delete+re-add; the check sits *before* the `!ruleChg` short-circuit so an engine-only PUT gets the exact message | `kvEngineImmutabilityCheck`, `rules.go:2613`, wired at `:3023` |
| Same-VIP mix | two rules on one VIP IP with different engines are **ACCEPTED** (that *is* the coexistence story) + one WARN naming both engines | `kvEngineMixDetect`, `rules.go:2626`, wired at `:2925` |
...
| `[KV_ZEROHIT]` | Go, `ai_kv_subscriber.go:1120` | watchdog WARN (once per transition edge) — the silent-parity-failure tripwire |
| `[KV_CONFIG]` | C, `sockproxy_http.c:2365` | rule config landing in the data plane, now incl. `kv_engine_type= kv_dp_rank_count= kv_svc_id=` |
...
is discarded (finding); the CICD scenario relaunches loxilb with stderr captured
...
|---|---|---|---|
| `loxilb_pd_kv_zero_hit_watchdog_total` | `service_id` | counter | **NEW :** lookups at-or-past the consecutive-zero-hit threshold against a non-empty inventory. Nonzero = the authoritative silent hash-parity-failure signal |
...
| `loxilb_pd_kv_tier15_miss_reason_total` / `t15_fallthrough_total` | `reason` / — | counter | unchanged guard-ladder misses (doc 08 §7.1) — on a single-role rule a fallthrough lands in the rule's own selector, not P/D Tier 2 |
...
| `loxilb_pd_kv_blocks` | `endpoint` | gauge | unchanged — reports the shared per-EP union inventory |
...
**Be precise about this.** waves 1–4 (plans) are authored and
...
| Gate | Result |
|---|---|
| Single-TU C unit harness (`test_kv_exact`, darwin clang + homebrew OpenSSL) | **153/153 PASS** — 107 wave-1 baseline + SGLang parity vectors + svc-id threading; mode-1 mask byte-identity **mutation-proven** (forced widening ⇒ FAIL `mask=0xf want 0x9`) |
| `gofmt` on every touched Go file; structural greps per plan | clean / PASS |
| `bash -n` + `shellcheck -S error` on all CICD + provisioning scripts | PASS |
...
6. Byte-identity re-run of the untouched `vllm-kvcache-routing-cpu` scenario (the shared
   mock publisher's default modes must stay green), plus the folded strict-gate checks
...
2. **`kvHashAlgo` must be omitted for SGLang rules via REST** — the swagger enum has no
   `sha256_sglang` value; the engine default is the only correct spelling (§8.1).
3. **`kvEngineType` is immutable** — change = delete + recreate; a PUT returns the exact
    rejection, never a silent re-add (§8.3).
4. **GUARD_B warmup is inert in production** (both paths — A1 finding, §3.3 note):
...
| Area | Files (roles) |
|---|---|
| Mode/enum constants + struct fields | `loxilb-ebpf/common/sockproxy.h` — `KV_EXACT_MODE_SINGLE_ROLE 3` (:435), `kv_engine_type`/`kv_dp_rank_count` in BOTH structs (:552-553, :1155-1156), `kv_svc_id` (:565), A1 warmup-inert note (:542-549), pfe `kv_sr_*` fields |
| Single-role selection branch + load lifecycle | `loxilb-ebpf/common/sockproxy_ep.c` — sibling branch (:741), inc-on-hit + `[KV_SR]` (:775-785), connect-fail release (:847-861) |
| Teardown decrement (single owner) | `loxilb-ebpf/common/sockproxy_http.c` — `pd_cleanup` release (:3516-3531); `kv_svc_id`/engine/rank stamped at both `proxy_add_entry` copy sites (:1963, :2364) + `[KV_CONFIG]` log |
| SGLang hash arm + mask widening + svc-id call | `loxilb-ebpf/common/sockproxy_kv_exact.c` — `kv_hash_sglang_block` (:475), dedicated SGLang loop + FIRST-8 truncation (:535-560), mask disjunct (:837-841), `kv_svc_id` at the CGO call (88); enum in `sockproxy_kv_exact.h:40` |
| CGO prototype (twin-lockstep) | `loxilb-ebpf/common/sockproxy_ai_gw.h:392` (+ stubs `sockproxy_ai_gw_stub.c`, `test_kv_exact.c`) |
...
| Go gate + engine guards | `pkg/loxinet/rules.go` — `KvExactModeSingleRole` (:2537), `kvSubscriberTargets` (:2552), `kvEngineConfigValidate`/`ImmutabilityCheck`/`MixDetect` (:2596/:2613/:2626), validations (:2867-2885), rank loops (:3582, :3604-3606), `KvSubscriberStopAll` (:3782) |
| Subscriber: KvEventSource / multi-rank / / watchdog / svc filter | `pkg/loxinet/ai_kv_subscriber.go` — `kvResyncDecision` (:118), `kvEpRankKey` (:306), `kvSvcScanTargets` (:360), `KvEventSource`/factory (:512/:523), `runKvSubscriberLoopRank` + `rankLastSeq` + decision markers (50), `KvSubscriberStartRank` (:898), `kvZeroHitWatchdog` + env (:1062-1123), scan exit (:1226), `//export llb_ai_kv_best_worker` (:1259) |
...
| Config chain (REST → DP) | `api/swagger.yml:7077-7126`, `api/restapi/handler/loadbalancer.go`, `common/common.go`, `pkg/loxinet/dpbroker.go`, `pkg/loxinet/dpebpf_linux.go:1773-1789` (engine→algo defaulting, the ONE mapping site) |
| Metrics | `api/prometheus/sockproxy_metrics.go` — `loxilb_pd_kv_zero_hit_watchdog_total` (:429), `IncKvZeroHitWatchdog` (:823), `KvZeroHitWatchdogValue` (:831) |
| Hash source of record + regen | `cicd/vllm-kvcache-routing-cpu/sglang_hash_core.py` (pinned `d8ef76682e`), `scripts/compute_sglang_hash_refs.py` |
...
- the internal A/B runbook — the wave-4 live A/B execution
  checklist: arms, HARD self-confirm gates, reinit wrapper, restore decision point.
