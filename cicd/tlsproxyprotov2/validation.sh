...
# ---------------------------------------------------------------------------
# PPV2MODE=fullproxy : validate the L7 userspace sockproxy path (not the eBPF
# L4 dp_ins_ppv2 path). loxilb terminates the client connection and emits a
# PROXY protocol v2 header on the backend socket (sockproxy_conn.c). The plaintext
# 'proxy_protocol' backends log $proxy_protocol_addr, so a correct emission shows
# the REAL client IP (10.10.10.1) at the backend, cleanly parsed. This path is
# GSO-immune (stream-level send), so no MTU/offload gymnastics are needed.
# ---------------------------------------------------------------------------
if [[ "${PPV2MODE:-fullnat}" == "fullproxy" ]]; then
  vip="10.10.10.254"; vport="2020"; client_ip="10.10.10.1"; eps="l3ep1 l3ep2 l3ep3"

  for ep in $eps; do
    $dexec "$ep" sh -c ": > /var/log/nginx/access.log; : > /var/log/nginx/error.log" 2>/dev/null
  done

  # Several requests to cover the 3-endpoint round-robin.
  body=""
  for n in 1 2 3 4 5 6; do
    body=$($hexec l3h1 curl -s --max-time 10 "http://${vip}:${vport}/")
  done

  alog=""; broken=0
  for ep in $eps; do
    alog="${alog}$($dexec "$ep" cat /var/log/nginx/access.log 2>/dev/null)"$'\n'
    b=$($dexec "$ep" sh -c "grep -c 'broken header' /var/log/nginx/error.log 2>/dev/null" 2>/dev/null)
    broken=$((broken + ${b:-0}))
  done

  echo "  last response body: ${body:-<empty>}"
  echo "  backend proxy_protocol_addr log lines:"
  echo "$alog" | grep -E '\->' | sed 's/^/    /'
  echo "  backend 'broken header' PROXY-parse errors: $broken"

  ok=1
  echo "$body" | grep -qE "server[0-9]"                                   || ok=0  # real backend answered
  echo "$alog" | grep -qE "(^|[^0-9.])${client_ip}([^0-9.]|$)"            || ok=0  # real client IP propagated
  [[ "$broken" == 0 ]]                                                    || ok=0  # header parsed cleanly

  echo "=================================================================="
  if [[ $ok == 1 ]]; then
    echo "RESULT: L7 fullproxy PPv2 OK - backend saw real client ${client_ip} via a"
    echo "        clean PROXY protocol v2 header emitted by the userspace sockproxy."
    echo "=================================================================="
    echo SCENARIO-tlsproxyprotov2 [OK] "(fullproxy ppv2 verified)"
    exit 0
  else
    echo "RESULT: L7 fullproxy PPv2 FAILED. body='${body}' broken=${broken}"
    echo "        (expected a serverN body and ${client_ip} in a proxy_protocol_addr log line)"
    echo "=================================================================="
    echo SCENARIO-tlsproxyprotov2 [FAILED] "(fullproxy ppv2 not verified)"
    exit 1
  fi
fi
