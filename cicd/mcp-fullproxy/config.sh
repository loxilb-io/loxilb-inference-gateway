...
if [[ "$USE_CLI" == "1" ]]; then
  create_lb_rule llb1 10.10.10.254 --tcp=2020:8080 --endpoints=31.31.31.1:1,32.32.32.1:1 --select=rr --mode=fullproxy --security=https --session-header-name=mcp-session-id --host=10.10.10.254 --trace-type=mcp
else
...
# port 2021 -> persist
if [[ "$USE_CLI" == "1" ]]; then
  create_lb_rule llb1 10.10.10.254 --tcp=2021:8080 --endpoints=31.31.31.1:1,32.32.32.1:1 --select=persist --mode=fullproxy --security=https --session-header-name=mcp-session-id --host=10.10.10.254 --trace-type=mcp
else
  $dexec llb1 curl -X POST http://localhost:11111/netlox/v1/config/loadbalancer \
    -H "Content-Type: application/json" \
    -d '{
