...
| **AI Gateway dashboard entirely empty** despite AI traffic flowing (200s served) | the rule serving the traffic lacks AI accounting: sockproxy records `loxilb_ai_*` only on connections whose rule enables `ai_gw_mode` — i.e. `sse_mode` (`--sse-mode`) or `pd_disagg_mode` (`--pd-disagg`). A plain fullproxy rule serves but records nothing. Also beware: `loxicmd get lb -o json` does **not** round-trip these attributes — snapshot rules via `GET /netlox/v1/config/loadbalancer/all` (raw API) if you plan to replay them |
