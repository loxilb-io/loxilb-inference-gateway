...
type PolMod struct {
	...
	// and the policer currently shapes nothing. Ignored on create, and
	// excluded from persistence: datapath state must not round-trip through
	// a snapshot, and the canonical snapshot encoding stays unchanged.
	Attached bool `json:"-"`
}
