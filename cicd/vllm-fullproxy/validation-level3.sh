...
echo SCENARIO-vllm-fullproxy-CHWBL-Level3
...
if echo "$lb_config" | grep -qE '"chwbl_mean_load_factor":[[:space:]]*250' && \
   echo "$lb_config" | grep -qE '"chwbl_replication":[[:space:]]*200'; then
    echo "✓ CHWBL parameters verified: chwbl_mean_load_factor=250, chwbl_replication=200"
else
    echo "✗ CHWBL parameters not found in REST API response (chwbl_mean_load_factor:250, chwbl_replication:200 expected)"
    code=1
fi
...
if [[ $code == 0 ]]; then
    echo "SCENARIO-vllm-fullproxy-CHWBL-Level3 [OK]"
    echo ""
    ...
    echo "SCENARIO-vllm-fullproxy-CHWBL-Level3 [FAILED]"
fi
