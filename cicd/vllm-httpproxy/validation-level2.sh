...
echo SCENARIO-vllm-httpproxy-CHWBL-Level2
...
echo "  CHWBL L2 routing: EP1_delta=$D1  EP2_delta=$D2"
if ([ "$D1" -ge 10 ] && [ "$D2" -eq 0 ]) || ([ "$D2" -ge 10 ] && [ "$D1" -eq 0 ]); then
    echo "  ✓ CHWBL L2 radix-tree consistency verified: all requests to one backend (D1=$D1, D2=$D2)"
else
    echo "  ✗ CHWBL L2 radix-tree inconsistent: requests split across backends (D1=$D1, D2=$D2)"
    code=1
fi
...
if [[ $code == 0 ]]; then
    echo "SCENARIO-vllm-httpproxy-CHWBL-Level2 [OK]"
    echo ""
    ...
    echo "SCENARIO-vllm-httpproxy-CHWBL-Level2 [FAILED]"
fi
