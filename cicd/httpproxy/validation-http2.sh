#!/bin/bash
source ../common.sh
echo SCENARIO-httpproxy-tcplb-http2
...
do
    ...
    then
        ...
        then
            ...
            echo SCENARIO-httpproxy-tcplb-http2 [FAILED]
...
do
    res=$($hexec l3h1 ../common/http2/http-client/http-client -host ${servIP[k]}:2021)
    res=$(echo "$res" | xargs)
    echo $res

    # Extract server name from HTTP/2.0:serverX format
    srv=${res#HTTP/2.0:}
