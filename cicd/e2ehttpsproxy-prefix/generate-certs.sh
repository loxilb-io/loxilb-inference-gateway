...
# Script to generate test certificates for HTTPS/HTTP2 TLS testing
...
# This ensures all certs share a single trusted CA (required for client verification).

# Check if IP address is provided
if [ $# -eq 0 ]; then
    echo "Usage: $0 <server-ip> [hostname] [existing-ca-cert] [existing-ca-key]"
    echo "Example: $0 31.31.31.1 server1"
    echo "Example (reuse CA): $0 31.31.31.1 server1 ./shared/certs/ca.crt ./shared/certs/ca.key"
    exit 1
fi
...
if [ -n "$EXISTING_CA_CERT" ] && [ -n "$EXISTING_CA_KEY" ]; then
    # Reuse provided CA — copy it into this cert dir so callers can find ca.crt here too
