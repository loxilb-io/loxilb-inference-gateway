// Code generated by go-swagger; DO NOT EDIT.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the generate command

import (
	"net/http"

	"github.com/go-openapi/runtime/middleware"
)

// PostConfigPolicyHandlerFunc turns a function with the right signature into a post config policy handler
type PostConfigPolicyHandlerFunc func(PostConfigPolicyParams, interface{}) middleware.Responder

// Handle executing the request and returning a response
func (fn PostConfigPolicyHandlerFunc) Handle(params PostConfigPolicyParams, principal interface{}) middleware.Responder {
	return fn(params, principal)
}

// PostConfigPolicyHandler interface for that can handle valid post config policy params
type PostConfigPolicyHandler interface {
	Handle(PostConfigPolicyParams, interface{}) middleware.Responder
}

// NewPostConfigPolicy creates a new http.Handler for the post config policy operation
func NewPostConfigPolicy(ctx *middleware.Context, handler PostConfigPolicyHandler) *PostConfigPolicy {
	return &PostConfigPolicy{Context: ctx, Handler: handler}
}

/*
	PostConfigPolicy swagger:route POST /config/policy postConfigPolicy

# Create a new Policy QoS config

Configure a policer. Rule targets require an exact VIP:PORT:PROTO or [VIP]:PORT:PROTO key; egress port targets require enabled egress hooks. Targets may remain pending. Information changes can delete and recreate an existing policer; target-only changes conflict. Mode propagation, numeric narrowing, and effective burst/rate semantics have known implementation limitations; this is not an atomic general-purpose update.
*/
type PostConfigPolicy struct {
	Context *middleware.Context
	Handler PostConfigPolicyHandler
}

func (o *PostConfigPolicy) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	route, rCtx, _ := o.Context.RouteInfo(r)
	if rCtx != nil {
		*r = *rCtx
	}
	var Params = NewPostConfigPolicyParams()
	uprinc, aCtx, err := o.Context.Authorize(r, route)
	if err != nil {
		o.Context.Respond(rw, r, route.Produces, route, err)
		return
	}
	if aCtx != nil {
		*r = *aCtx
	}
	var principal interface{}
	if uprinc != nil {
		principal = uprinc.(interface{}) // this is really a interface{}, I promise
	}

	if err := o.Context.BindValidRequest(r, route, &Params); err != nil { // bind params
		o.Context.Respond(rw, r, route.Produces, route, err)
		return
	}

	res := o.Handler.Handle(Params, principal) // actually handle the request
	o.Context.Respond(rw, r, route.Produces, route, res)

}
