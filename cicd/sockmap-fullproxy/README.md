...
| `request_path_client.py` | raw HTTP/1.1 client, one request shape per mode (`split`, `stream`, `pipeline`, `chunked`, `sizes`, `special`, `abort`, `halfclose`, `halfslow`, `halfsplit`, `halfmid`, `halfinflight`, `halfpartial`, `idle`, `keepalive`, `echo`, `volume`) |
