...
if [[ "$USE_CLI" == "1" ]]; then
  create_lb_rule llb1 10.10.10.254 --tcp=2020:8080 --endpoints=31.31.31.1:1,32.32.32.1:1 --mode=fullproxy --host=10.10.10.254 --path-prefix=/v1/users --path-match-mode=prefix
else
...
if [[ "$USE_CLI" == "1" ]]; then
  create_lb_rule llb1 10.10.10.254 --tcp=2020:8080 --endpoints=33.33.33.1:1 --mode=fullproxy --host=10.10.10.254 --path-prefix=/v1/orders --path-match-mode=prefix
else
  $dexec llb1 curl -X POST http://localhost:11111/netlox/v1/config/loadbalancer \
    -H "Content-Type: application/json" \
    -d '{
...
if [[ "$USE_CLI" == "1" ]]; then
  create_lb_rule llb1 10.10.10.254 --tcp=2021:8081 --endpoints=31.31.31.1:1,32.32.32.1:1 --mode=fullproxy --host=10.10.10.254 --backend-protocol=http2 --path-prefix=/v1/users --path-match-mode=prefix
else
  $dexec llb1 curl -X POST http://localhost:11111/netlox/v1/config/loadbalancer \
    -H "Content-Type: application/json" \
    -d '{
...
if [[ "$USE_CLI" == "1" ]]; then
  create_lb_rule llb1 10.10.10.254 --tcp=2021:8081 --endpoints=33.33.33.1:1 --mode=fullproxy --host=10.10.10.254 --backend-protocol=http2 --path-prefix=/v1/orders --path-match-mode=prefix
else
  $dexec llb1 curl -X POST http://localhost:11111/netlox/v1/config/loadbalancer \
    -H "Content-Type: application/json" \
    -d '{
