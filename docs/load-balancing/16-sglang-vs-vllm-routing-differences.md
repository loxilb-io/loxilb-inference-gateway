...
|---|---|---|
| **Serving topology** | P/D disaggregation: `ep_role:1` prefill + `ep_role:2` decode, KV handoff over vLLM's own NIXL connector | **Both shapes**: single-role converged pool (`kvExactMode:3`), **or** SGLang's own P/D disaggregation (`pd_disagg_mode` + `kvEngineType:"sglang"` + `pdBootstrapPort`) — the gateway injects the bootstrap triple and dual-dispatches to prefill AND decode concurrently; KV flows engine-to-engine over mooncake/NIXL via the prefill's bootstrap server |
