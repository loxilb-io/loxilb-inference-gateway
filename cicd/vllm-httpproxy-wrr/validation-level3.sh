...
echo SCENARIO-vllm-httpproxy-CHWBL-wrr-Level3
...
echo "  CHWBL WRR L3 routing: EP1_delta=$D1  EP2_delta=$D2"
if ([ "$D1" -ge 15 ] && [ "$D2" -eq 0 ]) || ([ "$D2" -ge 15 ] && [ "$D1" -eq 0 ]); then
    echo "  ✓ CHWBL WRR L3 radix-tree consistency verified: all requests to one backend (D1=$D1, D2=$D2)"
else
    echo "  ✗ CHWBL WRR L3 radix-tree inconsistent: requests split across backends (D1=$D1, D2=$D2)"
    code=1
fi
...
if [[ $code == 0 ]]; then
    echo "SCENARIO-vllm-httpproxy-CHWBL-wrr-Level3 [OK]"
    echo ""
    ...
    echo "SCENARIO-vllm-httpproxy-CHWBL-wrr-Level3 [FAILED]"
fi
