...
# Pre-flight: verify https-client binary is built
if [[ ! -x ../common/http2/https-client/client ]]; then
    echo "SKIP: ../common/http2/https-client/client binary not found"
    echo "      Build it with: cd cicd/common/http2/https-client && make"
    echo "SCENARIO-httpsproxy-prefix-http2 [SKIP]"
...
do
    res=$($hexec l3h1 ../common/http2/https-client/client -key 10.10.10.1/key.pem --cert 10.10.10.1/cert.pem --cacert minica.pem -host ${servIP[k]}:2021/v1/users)
    res=$(echo "$res" | xargs)
    echo $res
    srv=${res#HTTP/2.0:}
    srv=${srv%%:*}
...
do
    res=$($hexec l3h1 ../common/http2/https-client/client -key 10.10.10.1/key.pem --cert 10.10.10.1/cert.pem --cacert minica.pem -host ${servIP[k]}:2021/v1/orders)
    res=$(echo "$res" | xargs)
    echo $res
    srv=${res#HTTP/2.0:}
    srv=${srv%%:*}
