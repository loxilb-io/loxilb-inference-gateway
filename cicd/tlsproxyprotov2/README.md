...
### L7 fullproxy variant

The same scenario also covers the **L7 userspace** PROXY-protocol-v2 path (a
different mechanism from the eBPF L4 `dp_ins_ppv2` above): in fullproxy mode
loxilb terminates the client connection and emits the v2 header on the backend
socket (`sockproxy_conn.c`), which is stream-level and therefore GSO-immune.

```
PPV2MODE=fullproxy ./config.sh      # fullproxy + --host + --ppv2en; plaintext proxy_protocol backends
PPV2MODE=fullproxy ./validation.sh  # asserts the backend saw the REAL client IP via a clean v2 header
./rmconfig.sh
```

The backends log `$proxy_protocol_addr`; validation passes only when the real
client IP (10.10.10.1) appears there and nginx reports no `broken header`.
